...
var settings = config.Alligator;
//var pposettings = config.PPO;

var Alligator = require('./indicators/Alligator.js');
...
/*   this.uplevel = config.CCI.thresholds.up;
  ...
  this.addIndicator('alli', 'Alligator', settings);
}
...
method.log = function() {
    ...
    if (typeof(alli.result) == 'boolean') {
        log.debug('Insufficient data available. Age: ', alli.age, ' of ', alli.deth);
         //log.debug('ind: ', alli.TP.result, ' ', cci.TP.age, ' ', cci.TP.depth);
        return;
    }
    ...
    if (typeof(alli.result) == 'boolean' )
        ...
        log.debug('\t', 'Alli:\t', alli.result.toFixed(2));
}
